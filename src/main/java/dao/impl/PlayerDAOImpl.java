...
import dao.databaseConnection.DatabaseConnectionManager;
...
public PlayerDAOImpl() {
    ...
    public Optional<Player> findById(Integer id) throws DAOException {
        String sql = "SELECT id, name, email FROM players WHERE id = ?";
    ...
    public Player update(Player player) throws DAOException {
        String sql = "UPDATE players SET name = ?, email = ? WHERE id = ?";
        try (Connection conn = connectionManager.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public List<Player> findByName(String name) throws DAOException {
        ...
        String sql = "SELECT id, name, email FROM players WHERE name LIKE ?";
        try (Connection conn = connectionManager.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public List<Player> findTopPlayers(int limit) throws DAOException {
        List<Player> players = new ArrayList<>();
        String sql = "SELECT id, name, email FROM players ORDER BY name DESC LIMIT ?";
        try (Connection conn = connectionManager.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    public List<Player> findAll() throws DAOException {
        List<Player> players = new ArrayList<>();
        String sql = "SELECT id, name, email FROM players";
        try (Connection conn = connectionManager.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
