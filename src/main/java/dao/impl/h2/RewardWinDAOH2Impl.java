...
public class RewardWinDAOH2Impl implements BaseDAO<RewardWin, Integer>, RewardWinDAO {
    ...
    public RewardWin create(RewardWin rewardWin) throws DAOException {
        String sql = "INSERT INTO " + NAME_OBJECT + " (idReward, idPlayer, description, dateDelivery, isActive) VALUES (?, ?, ?, ?, ?);";
    ...
    public Optional<RewardWin> findById(Integer id) throws DAOException {
        String sql = "SELECT idRewardWin, idReward, idPlayer, dateDelivery, isActive, description FROM " + NAME_OBJECT + " WHERE idRewardWin = ?;";
        try (Connection connection = connectionDAO.getConnection();
    ...
    public List<RewardWin> findAll() throws DAOException {
        ...
        String sql = "SELECT idRewardWin, idReward, idPlayer, dateDelivery, isActive, description FROM " + NAME_OBJECT + ";";
        try (Connection connection = connectionDAO.getConnection();
    ...
    public RewardWin update(RewardWin rewardWin) throws DAOException {
        String sql = "UPDATE " + NAME_OBJECT + " SET idReward = ?, idPlayer = ?, description = ?, dateDelivery = ?, isActive = ? WHERE idRewardWin = ?";
        try (Connection connection = connectionDAO.getConnection();
             PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, rewardWin.getIdReward());
            stmt.setInt(2, rewardWin.getIdPlayer());
            ...
            if (rows == 0) {
                String messageError = "No " + NAME_OBJECT + " found to update with ID: " + rewardWin.getId();
    ...
    public void deleteById(Integer id) throws DAOException {
        String sql = "DELETE FROM " + NAME_OBJECT + " WHERE idRewardWin = ?;";
        try (Connection connection = connectionDAO.getConnection();
             PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, id);
    ...
    public boolean isExistsById(Integer id) {
        String sql = "SELECT 1 FROM " + NAME_OBJECT + " WHERE idRewardWin = ?;";
        try (Connection connection = connectionDAO.getConnection();
             PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, id);
            ResultSet rs = stmt.executeQuery();
