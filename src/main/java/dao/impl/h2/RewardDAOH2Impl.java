...
public class RewardDAOH2Impl implements BaseDAO<Reward, Integer>, RewardDAO {
    ...
    public Reward create(Reward reward) throws DAOException {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            stmt.setBoolean(4, reward.isActive());
            stmt.executeUpdate();
    ...
    public Optional<Reward> findById(Integer id) throws DAOException {
        String sql = "SELECT idReward, name, isActive FROM ? WHERE idReward = ?";
    ...
    public Reward update(Reward reward) throws DAOException {
        String sql = "UPDATE ? SET name = ?, description = ?, isActive = ? WHERE idReward = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, nameObject);
            stmt.setString(2, reward.getName());
            stmt.setString(3, reward.getDescription());
    ...
    public void deleteById(Integer id) throws DAOException {
        String sql = "DELETE FROM ? WHERE idReward = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, nameObject);
            stmt.setInt(2, id);
    ...
    public boolean isExistsById(Integer id) {
        String sql = "SELECT 1 FROM ? WHERE idReward = ?";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setString(1, nameObject);
            stmt.setInt(2, id);
            ResultSet rs = stmt.executeQuery();
